...
module.exports = {
  ...
  let bilmem = [] 
   for(var i in levels){
     ...
     bilmem.push({id: levels[i].id,  name: user.username, level: levels[i].level, xp: levels[i].xp})
   }
     const embed = new MessageEmbed()
     .setColor("#3f007f")
     .setTitle("Seviye Sıralama")
     var i = 0;
  await bilmem.forEach(async s => { 
     await embed.addField(`${i + 1}. ${s.name}`, `**Level** - ${s.level || 0}\n**XP** - ${s.xp} / 100}`)  
    })
     let index = bilmem.map(s => s.id).indexOf(message.author.id)
